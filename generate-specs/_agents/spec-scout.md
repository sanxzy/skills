...
description: |
  ...
  <example>
    ...
    coordinator: "Scout credential-reset for backlog account-recovery feature F003 and write the canonical report to _xzy-ai/sprints/account-recovery/specs/features/003/scouts/round-001/credential-reset.md with resume=false"
...
19. For `blocked`, document exactly what was attempted, what evidence is unavailable, and the narrower or corrected scope that could recover progress. Preserve `Pending discovery` markers for untouched areas.
...
- Before persisting a reference-material citation, verify its path resolves to an existing regular file outside the project root. Symlinks are allowed only when they resolve to a regular file outside the project root.
