...
Before performing a PPTX operation, use the current conversation and established user preferences to understand the intended outcome. Do not use a fixed checklist; judge from context whether missing information could materially change the content, narrative, audience fit, visual direction, scope, or acceptance criteria.

### Resolve material ambiguity before execution

- If a material decision is unresolved, pause the PPTX work. Explain what is blocked and why it matters, gather the unresolved questions, and hand off explicitly to the `discussion` skill in the same conversation when supported.
- Pass the unresolved questions and only the context needed to understand them; do not make the user repeat decisions already established.
- Wait for `discussion` to reach explicit shared-understanding confirmation before resuming. Do not defer a material decision or proceed with an unconfirmed assumption.
- If the `discussion` handoff is unavailable, keep the workflow paused and tell the user exactly which questions must be resolved. Do not substitute assumptions.

### Proceed when intent is clear

- If the request is clear and has no material ambiguity, execute it directly without an unnecessary discussion.
- For non-material gaps, make a safe assumption and state it briefly so the user can correct it.
- Use relevant conversation context and prior preferences; do not re-ask decisions that are already resolved.

### Handle conflicts and constraints explicitly

- If user instructions conflict, surface the conflict, explain the consequence, and ask the user to choose. Do not silently choose based on recency or specificity.
- If a request conflicts with mandatory technical, safety, or quality constraints, explain the limitation and offer the closest safe alternative. Never knowingly produce a corrupt, unreadable, or materially noncompliant deck just to follow an unsafe instruction.
- If the user changes a confirmed decision during execution, re-evaluate its impact and pause affected work. Return to `discussion` when the change is material or creates a conflict.

### Validate against confirmed intent

Before delivery, verify that the result still matches the confirmed purpose, audience, content, visual direction, constraints, requested format, and discussion decisions. For deck-producing operations, do this alongside the existing content, file, and visual QA; for read or analysis operations, verify the requested content and scope directly. Fix deviations within scope before delivery; report any remaining deviation or limitation clearly.
